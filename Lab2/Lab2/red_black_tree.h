...
namespace rbt//red-black tree
{
	...
	public:
        ...
        std::size_t setSize();//sets size of node, size = 1 + left->size + right->size
        std::size_t getLeftSize();//returns left->size if there is it or 0
        std::size_t getRightSize();//returns right->size if there is it or 0
    };
	template<typename T>
	class RedBlackTree
	{
	private:	
        ...
        TreeNode<T>* searchByRank(TreeNode<T>* node,std::size_t rank);//returns a node by rank
	...
	public:
        ...
        TreeNode<T>* searchByRank(std::size_t rank); //returns node by rank
        std::size_t searchRank(TreeNode<T>* node);//returns rank by node
        ~RedBlackTree();
    };
...
namespace rbt
{
    ...
    template<typename T>
    std::size_t TreeNode<T>::getLeftSize()
    {
        if (left) return left->size;
        return 0;
    }
    template<typename T>
    std::size_t TreeNode<T>::getRightSize()
    {
        if (right) return right->size;
        return 0;
    }
    template<typename T>
    std::size_t TreeNode<T>::setSize()
    {
        std::size_t leftSize = 0;
        if (left) leftSize = left->setSize();
        std::size_t rightSize = 0;
        if (right) rightSize = right->setSize();
    ...
    TreeNode<T>::TreeNode(T value, Color color,
        ...
        value{ value }, color{ color }, parent{ parent }, left{ left }, right{ right }, size{ 0 }{}
    template<typename T>
    ...
    void RedBlackTree<T>::insert(T value)
    {
        ...
        newNode->parent = node;
        if (!node) root = newNode;      
        else if (newNode->value < node->value) node->left = newNode;        
    ...
    template<typename T>
    TreeNode<T>* RedBlackTree<T>::searchByRank(TreeNode<T>* node, std::size_t rank)
    {
        if (!node) return nullptr;
        std::size_t size = node->getLeftSize() + 1;
        if (size == rank) return node;
        else if (rank < size) return searchByRank(node->left, rank);
        else return searchByRank(node->right, rank - size);
    }
    template<typename T>
    TreeNode<T>* RedBlackTree<T>::searchByRank(std::size_t rank)
    {
        if (root) root->setSize();
        return searchByRank(root, rank);
    }
    template<typename T>
    std::size_t RedBlackTree<T>::searchRank(TreeNode<T>* node)
    {  
        if (!node) return -1;
        if (root) root->setSize();
        std::size_t rank = node->getLeftSize() + 1;
        TreeNode<T>* current = node;
        while (current != root)
        {
            if (current == current->parent->right)
                rank += current->parent->getLeftSize() + 1;
            current = current->parent;
        }
        return rank;
    }
